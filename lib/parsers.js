        ...
        return function(args, ready) {
            ...
            eter(it=function(piece) {
              try {
                  ...
                  if(!okay(current)) {
					  error = new Error('Could not resolve "' + piece + '" in "' + pieces.join('.') + '"');
					  //console.log('Could not resolve "' + piece + '" in "' + pieces.join('.') + '"');
					  //error = new Error('Could not resolve piece');
            ...
            eter(it=function(filtervar) {
                filtervar(context)(value, function(err, data) {
                    if(err) {
                        ...
                        error = new Error('data was "null" or "undefined"');
                        it.done();
					} else {
                        value = data;
                        it.next();
